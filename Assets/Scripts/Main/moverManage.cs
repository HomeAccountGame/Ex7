    ...
    void Update()
    {
        if (Vector3.Distance(otherObject.transform.position, index[index_counter]) < 0.1f && !getTarget)
        {
            ...
            if (scene_list[index_counter] == "gift")
            {
                ...
                button.SetActive(!button.activeSelf);
                button2.SetActive(!button2.activeSelf);
                musicControl.insMusic.Audio.PlayOneShot(musicControl.insMusic.gift);
            }
            ...
            else
            {
                ...
                musicControl.insMusic.Audio.PlayOneShot(musicControl.insMusic.bank);

            }
            index_counter += 1;
            index_counter = index_counter % index.Count;
            Debug.Log(index_counter);
        }

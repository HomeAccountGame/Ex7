...
public class EndOfMonth : MonoBehaviour
{
    ...
    [SerializeField] JumpScene jump;
    private int count_month = 0;
    public int dest_month = 24;
    ...
    public void end_of_month()
    {
        ...
        count_month++;
        if(count_month > dest_month)
        {
            jump.SwitchScene("end_game");
        }
    }
    private void Update_money(Dictionary<string, float> asset)
    {
        foreach (KeyValuePair<string, float> assets in asset)
        {
            ...
            Debug.Log("Asset Name: " + assetName + ", Asset Value: " + assetValue);
        }
